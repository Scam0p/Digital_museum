...
class App {
  ...
  onTouchUp(e?: MouseEvent | TouchEvent) {
    ...
    if (clientX !== undefined && clientY !== undefined) {
      ...
      // If duration is short and drag movement is minimal, it is a click
      if (duration < 300 && dist < 10) {
        const rect = this.container.getBoundingClientRect();
        const clickX = clientX - rect.left;
        const normalizedX = (clickX / rect.width) * 2 - 1;
        const webglX = normalizedX * (this.viewport.width / 2);

        // Find the card mesh that matches the click X coordinate
        const clickedMedia = this.medias.find(media => {
          const halfWidth = media.plane.scale.x / 2;
          return webglX >= media.plane.position.x - halfWidth && webglX <= media.plane.position.x + halfWidth;
        });

        if (clickedMedia && this.onCardClick) {
          const originalLength = this.mediasImages.length / 2;
          const originalIndex = clickedMedia.index % originalLength;
